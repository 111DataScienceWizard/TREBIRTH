...
# Get all timestamps across all collections
all_timestamps = []
...
all_dates = sorted(list(set([ts.date() for ts in all_timestamps])))

# Get timestamps for selected collections
selected_timestamps = []
for collection in selected_collections:
    docs = db.collection(collection).stream()
    for doc in docs:
        data = doc.to_dict()
        selected_timestamps.append(data['timestamp'])

# Filter and sort selected dates
selected_dates = sorted(list(set([ts.date() for ts in selected_timestamps])))

# Create a date range slider
if selected_dates:
    min_date, max_date = min(selected_dates), max(selected_dates)
else:
    min_date, max_date = min(all_dates), max(all_dates)

selected_date_range = st.slider('Select Date Range', min_value=min(all_dates), max_value=max(all_dates), value=(min_date, max_date), format="YYYY MMM DD")

# Filter data by selected date range
filtered_data = []
for collection in selected_collections:
    docs = db.collection(collection)\
             .where('timestamp', '>=', datetime.combine(selected_date_range[0], datetime.min.time()))\
             .where('timestamp', '<=', datetime.combine(selected_date_range[1], datetime.max.time()))\
             .stream()
    for doc in docs:
