...
def plot_time_domain_with_filter(data, filter_type, cutoff_freq):
    ...
    for column in columns:
        ...
        filtered_data = apply_filter(data[column], filter_type, cutoff_freq)
        fig, ax = plt.subplots()
...
# Function to plot signals in frequency domain with filter
def plot_frequency_domain_with_filter(data, filter_type, cutoff_freq):
    columns = data.columns
    for column in columns:
        st.write(f"## {column} - Frequency Domain with {filter_type} Filter (Cutoff Frequency: {cutoff_freq} Hz)")
        filtered_data = apply_filter(data[column], filter_type, cutoff_freq)
        frequencies, powers = fq(pd.Series(filtered_data))
...
def spectrogram_plot(data):
    ...
    for i, column in enumerate(columns):
        ...
        cbar = plt.colorbar(pcm, ax=ax, label='Intensity [dB]')  # Create color bar
        st.pyplot(fig)
