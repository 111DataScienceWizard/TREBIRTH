...
def apply_filter(data, filter_type, cutoff_freq, sampling_rate=100, stopband_attenuation=60, steepness=0.9999):
    ...
    if filter_type == 'LPF' or filter_type == 'HPF':
        ...
        b = signal.firwin(order, cutoff_freq, window='hamming', pass_zero=pass_zero, nyq=nyquist_freq, cutoff=normalized_cutoff_freq, fs=sampling_rate, width=steepness)
    elif filter_type == 'BPF':
        normalized_cutoff_freq = (cutoff_freq[0] / nyquist_freq, cutoff_freq[1] / nyquist_freq)
        b = signal.firwin(order, cutoff_freq, window='hamming', pass_zero=False, nyq=nyquist_freq, cutoff=normalized_cutoff_freq, fs=sampling_rate, width=steepness)
